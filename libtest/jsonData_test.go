...
func TestJsonDataRename(t *testing.T) {
	...
	testNavIndex(t, jd, "UserA", "[UserA|assets UserA|pwHints]")
	...
	testNavIndex(t, jd, "RenameA", "[RenameA|assets RenameA|pwHints]")
	testNavIndex(t, jd, "Stuart", "[Stuart|assets Stuart|pwHints]")
	testNavIndex(t, jd, "Stuart|pwHints", "[Stuart|pwHints|application]")
	testNavIndexNot(t, jd, "UserA")
	testNavIndexNot(t, jd, "UserA|pwHints")
	testNavIndex(t, jd, "UserB", "[UserB|assets UserB|pwHints]")
...
func updateMap(a string, c *parser.Path, e error) {
	// if e == nil {
